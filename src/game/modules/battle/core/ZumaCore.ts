class ZumaCore extends egret.Sprite {

	//发射出来的小球
	private _ballShooted: any[];
	//做小球的挤入效果时存储需要被检测碰撞的小球
	private _ballCrushed: any[];
	//被吸引回退的第一个球的数组
	private _ballAttracted: any[];
	//最大连击数
	private _maxCobom: number = 0;
	//当前连击数
	private _cobom: number = 0;
	//游戏是否结束
	private _canShoot: boolean = true;
	//计算额外加分的时候要用到
	private _lastId: number;
	/** 存储球链中所有的小球 */
	private _rollBalls: Ball[];
	//开始滚入的小球的个数
	private _rollInCount: number = 35;
	//游戏中球的总数
	private _rollInTotalCount: number = 100;
	/** 角色 */
	private _role: Role;
	/** 关卡数据 */
	...
	public init(role: Role, levelVO: LevelVO): void {
		let self = this;
		self._role = role;
	...
	private ballRollIn() {
		...
		if (self._rollBalls.length < this._rollInCount) {
			for (let i: number = 0; i < self._rollBalls.length; ++i) {
				//所有小球移动4个位置
	 ...
	 * 在使用回退道具时要传入负参数
	 */
	public pushBallFrom(index: number, step: number): void {
		let self = this;
		var temp: Ball[] = [];
		temp.push(self._rollBalls[index]);
		//算出所有与ballArray中索引为index的小球"相连"的小球
		for (var i: number = index; i < self._rollBalls.length - 1; ++i) {
	...
	/** 游戏结束，小球进终点后消除 */
	public ballRollOut(): void {
		let self = this;
		for (var i: number = self._rollBalls.length - 1; i >= 0; --i) {
	...
	public shootBall(): void {
		let self = this;
		//发射球时的弧度
		var radian: number = (self._role.rotation + 90) * Math.PI / 180;
		var _ball: Ball = self._role.getCurrentBall();
	...
	private onShootBallMove(): void {
		let self = this;
		if (self._ballShooted.length != 0) {
			//这个循环对发射出来的每个球都进行检测
			for (var i: number = 0; i < self._ballShooted.length; ++i) {
				//这里检测是否越界
				if (self._ballShooted[i][0].x > 0 && self._ballShooted[i][0].x < App.StageUtils.getWidth() && self._ballShooted[i][0].y > 0 && self._ballShooted[i][0].y < App.StageUtils.getHeight()) {
					//检测碰撞
					var flag: number = self.cheackCollision(self._ballShooted[i]);
					if (flag == -1) {
						...
						var _ball: Ball = self._ballShooted[i][0];
						var radians: number = self._ballShooted[i][1];
						var dis: number = Math.sqrt((self._rollBalls[flag].x - _ball.x) * (self._rollBalls[flag].x - _ball.x) + (self._rollBalls[flag].y - _ball.y) * (self._rollBalls[flag].y - _ball.y));
						...
						*/
						//算出距前一个小球的位置的距离
						var PrevX: number = BattleConst.MapData[self._rollBalls[flag].posIndex - 16][0];
						var PrevY: number = BattleConst.MapData[self._rollBalls[flag].posIndex - 16][1];
						var prevDis: number = Math.sqrt((_ball.x - PrevX) * (_ball.x - PrevX) + (_ball.y - PrevY) * (_ball.y - PrevY));
						//算出距下一个小球的位置的距离
						var NextX: number = BattleConst.MapData[self._rollBalls[flag].posIndex + 16][0];
						var NextY: number = BattleConst.MapData[self._rollBalls[flag].posIndex + 16][1];
	...
	private cheackCollision(arr: any[]): number {
		let self = this;
		var _ball: Ball = arr[0];
		//这里检测是否与球链self._rollBalls中的球碰撞,碰撞采用距离计算
		for (var j: number = 0; j < self._rollBalls.length; ++j) {
			var dis: number = Math.sqrt((self._rollBalls[j].x - _ball.x) * (self._rollBalls[j].x - _ball.x) + (self._rollBalls[j].y - _ball.y) * (self._rollBalls[j].y - _ball.y));
			if (dis <= 32) {
				return j;
			}
		}
	...
	private insertBall(obj: Ball, index: number, p: String): void {
		let self = this;
		var posX: number;
		var posY: number;
		...
		if (p == "next") {
			...
			//第二种情况就是向前插入不会挤到前面的球，此时插入的球的位置应是被碰小球的posIndex-16个位置
			else {
				insertPos = self._rollBalls[index].posIndex - 16;
			}
		}
	...
	private cheackPushCollision(): void {
		let self = this;
		if (self._ballCrushed.length != 0) {
			for (var i: number = 0; i < self._ballCrushed.length; ++i) {
				var dis: number = Math.sqrt((self._ballCrushed[i][0].x - self._ballCrushed[i][1].x) * (self._ballCrushed[i][0].x - self._ballCrushed[i][1].x) + (self._ballCrushed[i][0].y - self._ballCrushed[i][1].y) * (self._ballCrushed[i][0].y - self._ballCrushed[i][1].y));
				var isCollision: boolean = dis < 32 ? true : false;
				var moveStep: number = 0;
				while (isCollision) {
	...
	private motionFinished(obj: Ball, insertPos: number): void {
		let self = this;
		var index: number;
		//算出应插入到self._rollBalls中位置的索引,这里采用传入insertPos参数的方式
		//而不是传入被碰撞小球的引用的原因是为了防止消除操作删除掉了小球而算不出
		//数组的插入位置
		for (var i: number = 0; i < self._rollBalls.length; ++i) {
	 ...
	 */
	private clearCheack(index: number, clear: boolean): number {
		let self = this;
		var temp: any[] = [];
		temp.push(self._rollBalls[index]);
		var color: number = self._rollBalls[index].color;
		//此循环向下搜索
		//是否要清除的参数为false则将不相连的也算起(为判断是否应取消连击计数提供依据)，否则不算，下同
		for (let i: number = index + 1; i < self._rollBalls.length; i++) {
	...
	private clearBall(f: number, arr: any[]): void {
		let self = this;
		//当前连击数加1
		++self._cobom;
		//播放的音调
		var id: number = self._cobom > 5 ? 5 : self._cobom;
	...
	private cheackColor(color: number): void {
		let self = this;
		//检查球链中的球
		for (var i: number = 0; i < self._rollBalls.length; ++i) {
			if (self._rollBalls[i].color == color) return;
		}
		//检查发射出来的球但还未插入的
		for (var j: number = 0; j < self._ballShooted.length; ++j) {
			if (self._ballShooted[j].color == color) return;
		}
		self._role.colorCleared(color);
	}

	/** 将需要被检测吸引的小球加入ballAttracted数组 */
	private addToBallAttracted(ball: Ball): void {
		let self = this;
	...
	private attract(): void {
		...
		if (self._ballAttracted.length != 0) {
			for (var i: number = 0; i < self._ballAttracted.length; ++i) {
				var index: number = self._rollBalls.indexOf(self._ballAttracted[i]);
				if (index != -1 && self._rollBalls[index - 1]) {
					if (self._ballAttracted[i].color == self._rollBalls[index - 1].color) {
						//算出应移动多少步
						var steps: number = self._ballAttracted[i].posIndex - self._rollBalls[index - 1].posIndex > 19 ? 3 : self._ballAttracted[i].posIndex - self._rollBalls[index - 1].posIndex - 16;
